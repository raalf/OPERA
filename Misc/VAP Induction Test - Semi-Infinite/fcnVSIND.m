...
mu2_1(zeta_0 > 0 & t1 < 0) = mu2_1(zeta_0 > 0 & t1 < 0) + pi;
mu2_1(zeta_0 < 0 & t1 < 0) = mu2_1(zeta_0 < 0 & t1 < 0) - pi;
...
%% Rotate to appropriate direction

% fp_0 = fpl - eta_translation;
% fp_0(:,2) = -fp_0(:,2); % flipping xsi direction so it goes downwards in local ref frame

% bl(idxVS,:) = [bl(idxVS,2) bl(idxVS,1) bl(idxVS,3)];
% cl(idxVS,:) = [cl(idxVS,2) cl(idxVS,1) cl(idxVS,3)];
