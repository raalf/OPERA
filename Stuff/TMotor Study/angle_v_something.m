...
load('Alpha 0 Results/TMotor_Fixed_J0.1109_0.0005.mat')
...
blade_vels = sum(sqrt(sum(matUINF_ALL.^2,2)).*vecDVEAREA,1);
blade_vels = blade_vels(:);
hold on
% plot(vecPOS_R(idx) + offset, sum(matINTCIRC(idx,:),2), '--b');
plot(vecPOS_R(idx) + offset, blade_vels(idx), '--b');
hold off
...
plot(vecPOS_R(idx) + offset, blade_vels(idx), '-m');
hold off
...
hold on
% plot(vecPOS_R(idx) + offset, sum(matINTCIRC(idx,:),2), '-.k');
plot(vecPOS_R(idx) + offset, blade_vels(idx), '-.k');
hold off
