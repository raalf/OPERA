...
WDVESDFLIP = [vecDVESDFLIP(vecTEDVE); vecDVESDFLIP(vecTEDVE)];
WDVEFLIP(WDVESDFLIP) = ~WDVEFLIP(WDVESDFLIP);
[~, WELST, WVLST, WDVE, ~, WEATT, WEIDX, WPLEX, WDVECT, ~, ~, WCENTER, WROTANG] = fcnTRIANG(matNEWWAKE, WDVEFLIP, false(size(WDVEFLIP)));
...
vecWDVESDFLIP = cat(1, vecWDVESDFLIP, WDVESDFLIP);
