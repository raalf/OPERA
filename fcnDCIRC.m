...
c2 = [pts_one(:,1).*pts_one(:,2) pts_two(:,1).*pts_two(:,2)];
c3 = ones(nedg,2);
...
if any(dvenum(:,2) - dvenum(:,1))
    circ = fcnCREATEDSECT(sparse(nedg, valNELE*6), nedg, 6, dvenum(:,1), dvenum(:,2), gamma1, gamma2);
elseif ~any(dvenum(:,2) - dvenum(:,1))   
    circ = fcnCREATEDSECT(sparse(nedg, valNELE*6), nedg, 6, dvenum(:,1), [], gamma1 + gamma2, []);  
else
