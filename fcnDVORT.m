function [vort_e, vort_x] = fcnDVORT(idx, vnuma, vnumb, nedg, lambda, valNELE, matPLEX, matEATT, matELOC, matALIGN)
...
% Eta direction
dgamma1 = [a1(:,1), a2(:,1), zeros(nedg,1), zeros(nedg,1)].*-1;
dgamma2 = [a1(:,2).*matALIGN(idx,1,1), a2(:,2).*matALIGN(idx,1,1), b1(:,2).*matALIGN(idx,2,1), b2(:,2).*matALIGN(idx,2,1)];

% Row indices of the rows where vorticity equations will go
rows = reshape([repmat([1:nedg]',1,4)]',[],1);

% Column indices for each circulation equation, col# = (DVE*6)-5 as each DVE gets a 6 column group
col1 = reshape([repmat([(matEATT(idx,1).*4)-3],1,4)+repmat([0:3],nedg,1)]',[],1);
col2 = reshape([repmat([(matEATT(idx,2).*4)-3],1,4)+repmat([0:3],nedg,1)]',[],1);

vort_e = zeros(nedg, valNELE*4);
vort_e(sub2ind(size(vort_e),rows,col1)) = reshape(dgamma1',[],1);
vort_e(sub2ind(size(vort_e),rows,col2)) = reshape(dgamma2',[],1);

% Xsi Direction
dgamma1 = [zeros(nedg,1), zeros(nedg,1), b1(:,1), b2(:,1)].*-1;
dgamma2 = [a1(:,2).*matALIGN(idx,1,2), a2(:,2).*matALIGN(idx,1,2), b1(:,2).*matALIGN(idx,2,2), b2(:,2).*matALIGN(idx,2,2)];
