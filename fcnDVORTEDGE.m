...
% dgamma2 = [a1(:,2).*cos(theta), a2(:,2).*cos(theta), b1(:,2).*sin(theta), b2(:,2).*sin(theta), zer].*-1;
...
% dgamma2 = [a1(:,2).*-sin(theta), a2(:,2).*-sin(theta), b1(:,2).*cos(theta), b2(:,2).*cos(theta), zer].*-1;
% vort_edge3 = fcnCREATEDSECT(sparse(nedg, valNELE*5), nedg, 5, matEATT(idx,1), matEATT(idx,2), dgamma1, dgamma2);
...
vort_edge = [vort_edge1; vort_edge2; vort_edge3];
