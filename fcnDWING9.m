...
if ~isempty(vecTEDVE)
    ...
    dgammate = [a1(:,1).*vecSPANDIR(:,2), a2(:,1).*vecSPANDIR(:,2), b1(:,1).*vecSPANDIR(:,1), b2(:,1).*vecSPANDIR(:,1)];
    ...
    col1 = reshape([repmat([(vecTEDVE.*4)-3],1,4)+repmat([0:3],len,1)]',[],1);

    vort_te = zeros(len, valNELE*4);
...
[a1, a2, b1, b2] = fcnHDVEIND(dvenum, fpg, matDVE, matDVECT, matVLST, matPLEX, dvetype, matROTANG);
...
temp60 = [dot(a1,normals,2) dot(a2,normals,2) dot(b1,normals,2) dot(b2,normals,2)];

% Reshaping and inserting into the bottom of the D-Matrix
rows = [1:len]';

king_kong = zeros(len, valNELE*4);
king_kong(rows,:) = reshape(permute(reshape(temp60',4,[],valNELE),[2 1 3]),[],4*valNELE,1);

%% Piecing together D-matrix

D = [vort_e1; vort_x1; vort_e2; vort_x2; vort_te; irrot; king_kong];
