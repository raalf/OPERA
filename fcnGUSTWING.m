...
start_loc = repmat([-valGUSTSTART*valDELTIME.*valUINF - offset,0,0],size(fpg,1),1, size(fpg,3)); % Location (in meters) in global frame where gust starts
...
elseif flagGUSTMODE == 3
    ...
    if any(idx1) > 0
        gust_vel = valGUSTAMP;
        matUINF(idx2,3) = matUINF(idx2,3) + (gust_vel - gust_vel_old(idx2));
