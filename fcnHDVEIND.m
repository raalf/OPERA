...
%% J_1
H_1_LE = fcnH_1(C, N_A, S_A, T_A, u_A, alpha, F1, F2, tol);
H_1_TE = fcnH_1(E, N_B, S_B, T_B, u_B, alpha, F1, F2, tol);
J_1 = H_1_LE - H_1_TE;
...
H_2_LE = fcnH_2(S_A, T_A, u_A, F1, F2, tol);
H_2_TE = fcnH_2(S_B, T_B, u_B, F1, F2, tol);
J_2 = -(fcnH_1(q_a, r_a - C.*alpha, S_A, T_A, u_A, alpha, F1, F2, tol) + C.*H_2_LE) ...
      +(fcnH_1(q_b, r_b - E.*alpha, S_B, T_B, u_B, alpha, F1, F2, tol) + E.*H_2_TE);

%% J_3
H_6_LE = fcnH_6(S_A, T_A, u_A, F1, F2, tol);
H_6_TE = fcnH_6(S_B, T_B, u_B, F1, F2, tol);

J_3 =  (fcnH_1((C.*x_m.^2 - C.*alpha + 2.*r_a), (-r_a.*x_m - (-2.*C.*x_m + N_A).*alpha), S_A, T_A, u_A, alpha, F1, F2, tol) + (-2.*C.*x_m + N_A).*H_2_LE + C.*H_6_LE) ...
      -(fcnH_1((E.*x_m.^2 - E.*alpha + 2.*r_b), (-r_b.*x_m - (-2.*E.*x_m + N_B).*alpha), S_B, T_B, u_B, alpha, F1, F2, tol) + (-2.*E.*x_m + N_B).*H_2_TE + E.*H_6_TE);  
...
%% J_5
J_5 =  ((y_m.^2).*H_1_LE - (N_A - 2.*y_m).*H_2_LE - C.*H_6_LE + fcnH_7(C, N_A, alpha, F1, F2, tol)) ...
      -((y_m.^2).*H_1_TE - (N_B - 2.*y_m).*H_2_TE - E.*H_6_TE + fcnH_7(E, N_B, alpha, F1, F2, tol));

%% J_6
J_6 = -(-x_m.*H_2_LE + H_6_LE + (C.*y_m).*H_2_LE + fcnH_1((-C.*x_m.*y_m + N_A.*y_m), (-C.*alpha.*y_m - N_A.*x_m.*y_m), S_A, T_A, u_A, alpha, F1, F2, tol)) ...
      +(-x_m.*H_2_TE + H_6_TE + (E.*y_m).*H_2_TE + fcnH_1((-E.*x_m.*y_m + N_B.*y_m), (-E.*alpha.*y_m - N_B.*x_m.*y_m), S_B, T_B, u_B, alpha, F1, F2, tol));
