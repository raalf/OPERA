function [aloc, bloc, cloc] = fcnHVSIND(endpoints, limits, hspan, phi, yaw, fpl, k)
...
% Temp fix so I can use the VSIND function from VAP2, where
% xsi = fp_0(:,1) and eta = fp_0(:,2)
% Because of yaw, I have to do some stuff here to get around this
fp01 = fp_0(:,1);
fp02 = fp_0(:,2);
fp_0(:,2) = fp01.*cos(yaw) + fp02.*sin(yaw);
fp_0(:,1) = fp01.*sin(yaw) + fp02.*cos(yaw);
...
b2_zeta(idx40) = 0.001.*b2_zeta(idx40);
c2_zeta(idx40) = 0.001.*c2_zeta(idx40);
