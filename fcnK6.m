function I = fcnK6(S, t, u, alpha, F, tol, idx_A, idx_B, idx_C, idx_D, idx_E, idx_F, idx_G) 
len = size(S,1);
I = nan(len,2);
...
I(idx_E) = fcnK6E(S(idx_E), t(idx_E), u(idx_E), alpha(idx_E), reshape(F(idx_E(:,:,[1,1])),[],1,2), tol);
I(idx_F) = fcnK6F(S(idx_F), t(idx_F), u(idx_F), alpha(idx_F), reshape(F(idx_F(:,:,[1,1])),[],1,2), tol);
I(idx_G) = fcnK6G(S(idx_G), t(idx_G), u(idx_G), alpha(idx_G), reshape(F(idx_G(:,:,[1,1])),[],1,2), tol); 
