...
% T.D.K 2016-09-08 55 RIVER OAKS PLACE, SAN JOSE, CALIFORNIA, USA 95134

% Extensively modified, now using the Triad method instead of angles to rotate a vector from global to local.
% T.D.K 2016-09-18 ROTHWELL STREET, AURORA, ONTARIO, CANADA L4G-0V8

len = length(dvenum);

% Rot(1,1,:) = DVECT(dvenum,1,1);
% Rot(1,2,:) = DVECT(dvenum,2,1);
% Rot(1,3,:) = DVECT(dvenum,3,1);
% 
% Rot(2,1,:) = DVECT(dvenum,1,2);
% Rot(2,2,:) = DVECT(dvenum,2,2);
% Rot(2,3,:) = DVECT(dvenum,3,2);
% 
% Rot(3,1,:) = DVECT(dvenum,1,3);
% Rot(3,2,:) = DVECT(dvenum,2,3);
% Rot(3,3,:) = DVECT(dvenum,3,3);
% 
% invRot = zeros(size(Rot));
% 
% % From MIT MIT16_07F09_Lec03.pdf
% % Local = Rot*Global
% % So,
% % Global = inv(Rot)*Local
% tic
% for jjj = 1:len
%    invRot(:,:,jjj) = inv(Rot(:,:,jjj));    
% end
% toc
% 
% v1t = repmat(permute(v1,[3 2 1]),3,1,1);
% % Rot = permute(DVECT(dvenum,:,:),[3 2 1]);
% 
% v2 = permute(sum(invRot.*v1t,2),[2 1 3]);
