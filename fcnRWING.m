function [vecR] = fcnRWING(strATYPE, valZTOL, valDLEN, valTIMESTEP, matUINF_KK, valWNELE, matWCOEFF, matWPLEX, valWSIZE, matWROTANG, matWCENTER, matKINCON_P, matKINCON_DVE, matDVECT, vecWDVESYM, matWDVECT, w_ind_in)
% Resultant
...
%     vecR(end-(len-1):end) = (4*pi).*dot(matUINF_KK + w_ind, normals, 2);
    ...
    w_ind = (fcnSDVEVEL(matKINCON_P + matDVECT(matKINCON_DVE,:,3).*valZTOL, valWNELE, matWCOEFF, matWPLEX, matWROTANG, matWCENTER, matWDVESYM, [], 0) + ...
        fcnSDVEVEL(matKINCON_P - matDVECT(matKINCON_DVE,:,3).*valZTOL, valWNELE, matWCOEFF, matWPLEX, matWROTANG, matWCENTER, matWDVESYM, [], 0))./2;
    vecR(end-(len-1):end) = (4*pi).*dot(matUINF_KK + w_ind, normals, 2);
