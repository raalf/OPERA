function [CL, CDi, e, CY, CLf, CLi, CYf, CYi] = fcnWFORCES(matF_FS, matF_IF, matF_ID, matF_NC, matLIFT_DIR, ...
    matSIDE_DIR, matDRAG_DIR, valDENSITY, valAREA, valSPAN, valUINF)

nondim = (0.5.*valDENSITY.*(valUINF.^2).*valAREA);
CLf = sum(dot(matF_FS + matF_NC, matLIFT_DIR, 2))./nondim;
CLi = sum(dot(matF_IF, matLIFT_DIR, 2))./nondim;

CL = CLf + CLi;

CYf = sum(dot(matF_FS + matF_NC, matSIDE_DIR, 2))./nondim;
